...
function injectHook(options, name, callback) {
    ...
    var className = options.constructor && options.constructor.name;
    var hookName = className ? SAN_HOOK_ORIGIN + '.' + className : SAN_HOOK_ORIGIN;
    // 防止多次热更新之后出现套娃
    if (existing && existing[hookName]) {
        return;
    }
